...
gearFromFn cacheT forkedCache gearFn = do
  serialized ← unstableSerialized (serGearFn cacheT gearFn)
  gearsFnIndexV ← asks envGearsIndex
  cache ← tryLazy gearsFnIndexV \gearsFnIndex →
    RT.upsertChurch (RT.selEq serialized) gearsFnIndex <&> \case
...
reconfGear ∷ (Typeable ctx, InferValT True ctx, Typeable out) ⇒ Gear ctx out → ctx → AppIOC (Gear ctx out)
...
confGear ∷
  (InferValT True ctx {-InferValT True cfg, InferValT True cache, Ser cache,-}, Typeable ctx, Typeable out) ⇒
...
runGear (UnsafeGear cacheT@(valTypeableProof → Dict) (GearFn cfgT cfg (UnsafeGearTemplate _ _ fn)) cacheInd) = do
  gearsV ← asks envGears
...
asmCached ∷ (InferValT True cache) ⇒ cache → Asm ctx (cache → AppIOC (out, cache)) → Asm ctx out
...
-}
bucket ∷
  (RT.IsRadixKey k, RT.IsRadixKey bucketK, Foldable t, InferContainerT c1, InferContainerT c2, InferContainerT c3, InferValT True bucketK, InferValT True v, InferValT True k, InferValT True ctx, Ser v, Container c2, Container c3, Container c1, Typeable k, Typeable ctx, Typeable bucketK) ⇒
