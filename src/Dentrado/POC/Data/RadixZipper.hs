...
reduceInvChunk'' :: (Container c, Has (Writer (Reduced' s)) sig m) => Proxy s -> InvRadixChunk' c a -> m (InvRadixChunk' c a, Maybe (c (InvRadixChunk c a)))
...
reduceInvChunk' :: (Container c, Has (Writer (Reduced' s)) sig m) => Proxy s -> InvRadixChunk' c a -> m (InvRadixChunk' c a)
...
allocReducedInvChunk :: (Container c1, Container c2, Has Fresh sig m) => (forall b. c2 b -> WriterC Reduced m (c1 b)) -> InvRadixChunk' c2 a -> m (c1 (InvRadixChunk c2 a))
