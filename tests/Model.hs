...
import Dentrado.POC.Memory (AppIOC (..), EnvLoad (..), EnvStore (..), Env (..), Val (..), ValT (..), sendAI)
import Dentrado.POC.Types (EventId (..), Event (..), Timestamp (..), LocalId (..), Any1 (..), SiteAccessLevel (..))
...
import Dentrado.POC.Gear (builtinAsmGearTemplate, GearTemplate', asmGear, events, runGear, confNewGear)
...
status = $sFreshI $ builtinAsmGearTemplate $
  SG.mkStateGraph
    (asmGear events)
    (StateGraphDepsNil, \case
...
prop_test1_oneshot_correct = withMaxSuccess 1 $
...
prop_test1_multishot_correct = withMaxSuccess 100 $ forAll
  ...
  \test1' -> test1Res == unsafeRunAppIO do
    -- traceM "running new"
    status' <- confNewGear status ()
