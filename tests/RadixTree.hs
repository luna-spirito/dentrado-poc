...
data MapLikeCommand v = Insert !v | Delete
  deriving
    ( -- | Focus -- todo: Pop
      Show
    )
...
instance (Arbitrary v) ⇒ Arbitrary (MapLikeScript v) where
  ...
  shrink (MapLikeScript _keys cmds) = do
    ...
    pure
      $ MapLikeScript
        ( Set.fromList $ cmds' >>= \cmd → case fst cmd of
            SelEq k → [k]
            _nonSpecific → []
        )
        cmds'
...
processScriptMap =
  processScript
    ...
    ( \case
        ...
        SelMax → pure . fmap snd . Map.lookupMax
    )
    Map.empty
...
processScriptRadixTree =
  processScript
    ...
    ( \case
        ...
        SelMax → fmap join . RT.runNonDetMax . RT.lookup RT.selNonDet
    )
    RT.empty
